...
def getInfobox(name,link=False):  
    try:  
        ...
        lemma_summary = driver.find_element_by_class_name("lemma-summary") 
        summary = re.sub(r"(\[.*?\])",'',lemma_summary.text,re.M)
        update_status_of_words(name,summary)
        update_status_of_links(name)
        
        content = ''
        para = driver.find_elements_by_xpath("//div[@class='para']") 
        for p in para:
            p_text = str(p.text)
            p_text = re.sub(r"(\[.*?\])",'',p_text,re.M)
            # brackets = re.findall(r"(\[.*?\])",p_text,re.M)
            # for bracket in brackets:
                # print(bracket)
                # p_text = p_text.replace(bracket,"")
            # if p_text.find('，') == -1 and p_text.find('、') == -1 and p_text.find('。') == -1:
            #     continue
            # if p_text.find("。") != -1:
            #     # print(p_text)
            #     content += p_text
...
def main():
    try:
        ...
        source = fetch_links_with_num(1000,300000)
        while source:
            for name,link in source:  
                name = name.replace("\n", "")
                save_word_and_desc(str(name),'none','root')
                getInfobox(name,link)  
            source = fetch_links_with_num(1000,100000)
            repair_table()
        print ('End')  
    except:
        traceback.print_exc()  
